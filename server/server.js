...
function requireLogin(req, res, next) {
  if (req.session && req.session.user) {
    ...
    //user is not authenticated, 401 unautorized
    res.sendStatus(401);
  }
}
  ...
  app.get('/logout', function (req, res) {
    req.session.destroy(function(err) {
      ...
      //closing session by clearing cookie on client
...
app.put("/movies/:imdbID", requireLogin, function (req, res) {
  ...
  if (!exists) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), config.omdbTimeoutMs || 5000);
    ...
    .then(data => {
      ...
      //converting the response to the movie format
      const formattedMovie = {
        ...data,
        Genres: data.Genre ? data.Genre.split(', '): []
      };
      //saving it to the users collection
...
console.log("Key:", config.omdbApiKey);
